...
class LivePortraitPipeline(object):
    ...
    def generate_frame(
        ...
        R_d_i = get_rotation_matrix(
            x_d_i_info["pitch"] * self.pitch,
            x_d_i_info["yaw"] * self.yaw,
            x_d_i_info["roll"] * self.roll,
        )
        R_new = R_d_i @ R_s
        delta_new = (
            x_s_info["exp"] + (x_d_i_info["exp"] - x_s_info["exp"]) * self.expression
        )
        scale_new = (
            x_s_info["scale"] * (x_d_i_info["scale"] / x_s_info["scale"]) ** self.scale
        )
        ...
        if inference_cfg.flag_pasteback:
            ...
            I_p_i_to_ori_blend = paste_back(
                I_p_i, crop_info["M_c2o"], img_rgb, mask_ori
            )
