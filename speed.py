...
def load_and_compile_models(cfg, model_config):
    ...
    appearance_feature_extractor = load_model(
        cfg.checkpoint_F, model_config, cfg.device_id, "appearance_feature_extractor"
    )
    motion_extractor = load_model(
        cfg.checkpoint_M, model_config, cfg.device_id, "motion_extractor"
    )
    warping_module = load_model(
        cfg.checkpoint_W, model_config, cfg.device_id, "warping_module"
    )
    spade_generator = load_model(
        cfg.checkpoint_G, model_config, cfg.device_id, "spade_generator"
    )
    stitching_retargeting_module = load_model(
        cfg.checkpoint_S, model_config, cfg.device_id, "stitching_retargeting_module"
    )
...
def warm_up_models(compiled_models, stitching_retargeting_module, inputs):
    ...
    with torch.no_grad():
        for _ in range(10):
            ...
            compiled_models["Warping Network"](
                inputs["feature_3d"], inputs["kp_driving"], inputs["kp_source"]
            )
            compiled_models["SPADE Decoder"](
                inputs["generator_input"]
            )  # Adjust input as required
...
def print_benchmark_results(
    ...
    for index, retarget in enumerate(retargeting_models):
        num_params = sum(
            p.numel() for p in stitching_retargeting_module[retarget].parameters()
        )
        num_params_in_millions = num_params / 1e6
        print(
            f"Number of parameters for part_{index} in Stitching and Retargeting Modules: {num_params_in_millions:.2f} M"
        )

    for name, avg_time in average_times.items():
        std_time = std_times[name]
        print(
            f"Average inference time for {name} over 100 runs: {avg_time:.2f} ms (std: {std_time:.2f} ms)"
        )
...
def main():
    ...
    times, overall_times = measure_inference_times(
        compiled_models, stitching_retargeting_module, inputs
    )
